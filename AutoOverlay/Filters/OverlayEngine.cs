...
[assembly: AvisynthFilterClass(
    ...
    "[Configs]c[Presize]s[Resize]s[Rotate]s[Editor]b[Mode]s[SIMD]b[Debug]b",
...
namespace AutoOverlay
{
    public class OverlayEngine : OverlayFilter
    {
        ...
        protected override void AfterInitialize(ScriptEnvironment env)
        {
            ...
            sourcePrepared = Prepare(Source, env);
            overlayPrepared = Prepare(Overlay, env);
            ...
            var cacheKey = env.GetEnv2() == null ? CacheType.CACHE_25_ALL : CacheType.CACHE_GENERIC;
            ...
            if (Editor)
            {
                ...
                form?.Close();
                form = new OverlayEditor(this, env);
        ...
        private Clip Prepare(Clip clip, ScriptEnvironment env)
        {
            var colorSpace = clip.GetVideoInfo().pixel_type;
            if (clip.IsRealPlanar())
            {
                var function = colorSpace.GetBitDepth() == 8 ? "ConvertToY8" : "ConvertToY";
                return env.InvokeClip(function, clip);
            }
            return clip;
        }
            ...
            using (new DynamicEnviroment(StaticEnv))
                foreach (var _config in configs)
                {
                            ...
                            foreach (var best in subResultSet)
                            {
                                ...
                                if (!initStep)
                                {
                                    ...
                                    maxWidth = Math.Min(maxWidth,
                                        Round((best.Width + config.Correction) * coefDiff) + 1);
                                }
                    ...
                    for (var substep = 1; substep <= config.Subpixel; substep++)
                    {
                        ...
                        var testParams = new HashSet<TestOverlay>();
                        var bestCrops = subResults.TakeWhile((p, i) => i < config.Branches && p.Diff - subResults.Min.Diff < config.BranchMaxDiff).ToList();
            ...
            int minIntersectArea, double minOverlayArea, int branches)
        {
            ...
            var tasks = from test in testParams
                ...
                select new {src, srcMask, srcSize, over, overMask, overSize, testGroup, excess, activeCrop};
            ...
            Task.WaitAll(tasks.Select(task => Task.Factory.StartNew(() => Parallel.ForEach(task.testGroup,
                searchArea =>
                {
                    ...
                    stat.BaseWidth = task.overSize.Width;
                    stat.BaseHeight = task.overSize.Height;
